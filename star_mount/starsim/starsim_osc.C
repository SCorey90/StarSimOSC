  ...
  for (Int_t i = 0; i < n; i++) {
    ...
    for (Int_t tr = 0; tr < ntracks; tr++) {
      ...
      // case for pi+
        ...
        StarGenParticle *pion_plus = kinematics->AddParticle("pi+");
        double px  = vecArr[tr]->Px();
        double py  = vecArr[tr]->Py();
        double pz  = vecArr[tr]->Pz();
        pion_plus->SetPx(px);
        pion_plus->SetPy(py);
        pion_plus->SetPz(pz);
        pion_plus->SetEnergy(energy);
      }
      // case for pi-
      else if (idpart == -211){
        StarGenParticle *pion_minus = kinematics->AddParticle("pi-");
        double px  = vecArr[tr]->Px();
        double py  = vecArr[tr]->Py();
        double pz  = vecArr[tr]->Pz();
        pion_minus->SetPx(px);
...
void starsim(Int_t nevents, Int_t jobNum, const char* dir, Int_t rngSeed = 1234) {
  ...
  TString jobDir = TString::Format("%s/job%d/", dir, jobNum);
